#!/usr/bin/env python3
"""
Multi-Tab Claude Voice Assistant with Auto-Approval
Based on multi_tab_voice_http.py with auto-approval from voice_tts_realtime.py
"""
...
# Auto-approval patterns from voice_tts_realtime.py
permission_patterns = [
    r'❯\s*1\.\s*yes',
...
# Track approval cooldowns per tab
approval_cooldowns = {}  # tab_id -> last_approval_time
...
def check_for_permission_prompt(content, tab_id):
    ...
    if tab_id in approval_cooldowns:
        if current_time - approval_cooldowns[tab_id] < 3:  # 3 second cooldown
            return False
    ...
    if prompt_detected:
        ...
        if session:
            ...
            approval_cooldowns[tab_id] = current_time
...
<html>
<head>
    <title>Multi-Tab Claude Voice Assistant (Auto-Approve)</title>
...
<body>
    ...
    <div id="status"></div>
    <div class="auto-approve-indicator">Auto-Approval Active</div>
...
if __name__ == '__main__':
    ...
    print("\n" + "="*60)
    print("🎙️ MULTI-TAB CLAUDE VOICE ASSISTANT WITH AUTO-APPROVAL")
    print("="*60)
    print("✨ Features:")
    print("  - Up to 4 simultaneous Claude sessions")
    print("  - Auto-approval for bash commands")
    print("  - Tab switching with real-time stats")
    print("  - HTTPS support for secure microphone access")
    print("")
    print("📱 Access at: https://192.168.40.232:8405")
    print("="*60 + "\n")
    ...
    if os.path.exists('cert.pem') and os.path.exists('key.pem'):
        ...
        print("📱 Access at: http://192.168.40.232:8405")
        socketio.run(app, 
                     host='0.0.0.0',
                     port=8405,
                     debug=False,
