...
<body>
    ...
    <script>
        ...
        let voiceQueue = [];
        let isPlaying = false;
        let tabTimers = {}; // tabId -> { intervalId, startTime }
        ...
        function switchTab(tabId) {
            ...
            if (targetTab) {
                ...
                // Clear notification states when clicking on tab
                targetTab.classList.remove('notification-yellow', 'notification-orange', 'notification-red');
            } else {
                console.error(`Tab ${tabId} not found in DOM!`);
            }
            
            // Clear timer for the tab we just clicked (since it's now active)
            clearTabTimer(tabId);
            
            activeTabId = tabId;
            
            // Start timers for any tabs that don't have timers yet
            document.querySelectorAll('.tab').forEach(tab => {
                if (tab.id !== activeTabId && !tabTimers[tab.id]) {
            ...
            document.getElementById('realtimeTokens').textContent = '-';
        ...
        // Tab highlighting functions
        function startAllTabTimers() {
            // Start timers for all tabs except the active one
            document.querySelectorAll('.tab').forEach(tab => {
        ...
        function startTabTimer(tabId) {
            ...
            tabTimers[tabId] = {
                startTime: startTime,
                intervalId: setInterval(() => updateTabHighlight(tabId, startTime), 1000)
            };
        }
        ...
        function updateTabHighlight(tabId, startTime) {
            ...
            // Skip if this became the active tab
            if (tabId === activeTabId) {
                clearTabTimer(tabId);
                return;
            }
            ...
            // Apply appropriate notification class based on time
            if (minutes >= 10) {
        ...
        function clearAllTimers() {
            Object.keys(tabTimers).forEach(tabId => clearTabTimer(tabId));
        }
        ...
        function createDefaultTabs() {
            ...
            if (firstTabId) {
                ...
                // Start timers for all other tabs
                setTimeout(() => startAllTabTimers(), 100);
            }
...
if __name__ == '__main__':
    ...
    print("")
    print("📱 Access at: http://192.168.40.232:8401")
    print("="*60 + "\n")
    
    socketio.run(app, host='0.0.0.0', port=8401, allow_unsafe_werkzeug=True)