#!/usr/bin/env python3
"""
Multi-Tab Claude Voice Assistant with Stats Panel - Simple Version
4 tabs are automatically created and cannot be closed
"""
...
<html>
<head>
    <title>Multi-Tab Claude Voice Assistant - Simple</title>
    ...
    <div class="main-container">
        ...
        <div class="content-container">
            ...
            <div class="main-card">
                ...
                <div class="info">
                    4 permanent tabs • Real-time voice interaction • Stats tracking
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function createDefaultTabs() {
            ...
            function createNextTab() {
                if (tabsCreated < 4) {
                    const tabId = 'tab_' + tabsCreated;
                    const projectName = defaultNames[tabsCreated];
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            tabsCreated++;
                            setTimeout(createNextTab, 100); // Small delay between creates
                        }
                    });
                }
            }
        ...
        function toggleRecording() {
            if (!activeTabId) {
                alert('Please wait for tabs to load');
                return;
            }
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("🎙️ MULTI-TAB CLAUDE VOICE ASSISTANT - SIMPLE")
    print("="*60)
    print("✨ Features:")
    print("  - 4 permanent tabs (cannot be closed)")
    print("  - Real-time stats panel (TIME/TOKENS)")
    print("  - Tab renaming via double-click")
    print("  - Stats update for active tab only")
    print("")
    print("📱 Access at: http://192.168.40.232:8403/simple")
    print("="*60 + "\n")
    
    socketio.run(app, host='0.0.0.0', port=8403, allow_unsafe_werkzeug=True)