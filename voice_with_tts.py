#!/usr/bin/env python3
"""
Claude Voice Bot with Text-to-Speech Response
"""
...
def capture_tmux_output():
    ...
    while True:
        try:
            # Capture the last 50 lines from tmux
            ...
            # Check if there's new output after our command
            if current_output != last_output and last_command:
                # Look for new content after the last command
                if last_command in current_output:
                    # Find the position of our command
                    cmd_pos = current_output.rfind(last_command)
                    # Get everything after our command
                    new_content = current_output[cmd_pos + len(last_command):].strip()
                    
                    # Filter out prompts and empty lines
                    lines = []
                    for line in new_content.split('\n'):
                        line = line.strip()
                        # Skip empty lines, prompts, system messages, and token info
                        if (line and 
                            not line.startswith('$') and 
                            not line.startswith('Human:') and
                            not line.startswith('Assistant:') and
                            'The user sent the following message:' not in line and
                            'tokens' not in line.lower() and
                            'auto-accept' not in line.lower() and
                            'continuation' not in line.lower() and
                            not line.startswith('[') and
                            not line.endswith(']') and
                            'Press' not in line and
                            'Enter' not in line):
                            lines.append(line)
                    ...
                    if lines:
                        ...
                        if len(response) > 20:  # Only queue substantial responses
...
def test():
    ...
    try:
        test_message = "VOICE BOT TEST: TTS version working!"
...
if __name__ == '__main__':
    print("\n" + "="*50)
    print("🎙️  CLAUDE VOICE WITH TTS")
    print("="*50)
    print("✨ Features:")
    print("  - Voice input")
    print("  - Text-to-speech responses")
    print("  - Adjustable speech speed")
    print("  - Auto-speak toggle")
    print("")
    print("📱 Access at: https://192.168.40.232:8100")
    print("="*50 + "\n")
    ...
    app.run(host='0.0.0.0', port=8100, debug=False, ssl_context=context)